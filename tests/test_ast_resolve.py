...
def test_resolve_top_level_import_attr():
    a = ast.parse(
        r"""
from dataclasses import dataclass
import pyhparams
from pyhparams.ast_data_fields_resolve import RESOLVE
assigned = pyhparams.utils.UtilsTestParams(x=10, y= 1/139.)  
resolved = RESOLVE(pyhparams.utils.UtilsTestParams.a) # resolve last assignment a=10 instead of default 0
"""
    )
...
"""
    ...
    resolved = ast_to_dict(ast_resolve_dataclass_filed(a))
    assert resolved.get("assigned").b.nesed_class_level2.get("val") == 100
...
def test_resolve_nested_class_dict_level2():
    a = ast.parse(
        r"""
from dataclasses import dataclass
from pyhparams.ast_data_fields_resolve import RESOLVE
...
class A:
    ...
    b: B 
    a: int = 1
assigned = A(a=100, b =A.B({"val":RESOLVE(A.a)}))  
"""
    )
    resolved = ast_to_dict(ast_resolve_dataclass_filed(a))
    assert resolved.get("assigned").b.nesed_class_level2.get("val") == 100
...
def test_resolve_nested_data_class_defined_nested_with_dict_kw():
    a = ast.parse(
        r"""
from dataclasses import dataclass
from pyhparams.ast_data_fields_resolve import RESOLVE
...
class A:
    ...
    class B:
        b: int = 3
    nesed_class: Dict[str,B] = None
assigned = A(a=100, nesed_class = dict(val=A.B(b=RESOLVE(A.a))) )  
"""
    )
    resolved = ast_to_dict(ast_resolve_dataclass_filed(a))
    assert resolved.get("assigned").nesed_class.get("val").b == 100
...
def test_resolve_tuple_idx_assigned_to_dict():
    a = ast.parse(
        r"""
from dataclasses import dataclass
from typing import Tuple
...
class A:
    a_tuple: Tuple[int,int]= (10,20)                   
a = A(a_tuple=(1,2))
idx0 = {"foo":RESOLVE(A.a_tuple)[0]}
"""
    )
    resolved = ast_to_dict(ast_resolve_dataclass_filed(a))
    assert resolved.get("idx0").get("foo") == 1


def test_resolve_tuple_idx_assigned_to_dict_nested_in_dataclass():
    a = ast.parse(
        r"""
from dataclasses import dataclass
...
class A:
    b_dict: Dict[str,int] 
    a_tuple: Tuple[int,int]= (10,20)                   
a = A(a_tuple=(1,2), b_dict={"foo":RESOLVE(A.a_tuple)[0]})
"""
    )
    resolved = ast_to_dict(ast_resolve_dataclass_filed(a))
    assert resolved.get("a").b_dict.get("foo") == 1
